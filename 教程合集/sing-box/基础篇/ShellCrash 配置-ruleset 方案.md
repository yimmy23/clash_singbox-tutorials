...
连接 SSH 后执行如下命令：
```
curl -L https://cdn.jsdelivr.net/gh/DustinWin/clash_singbox-tools@main/sing-box-puernya/sing-box-linux-armv8.tar.gz | tar -zx -C /tmp/
```
# 二、 添加定时任务
...
9. 进入主菜单 -> 5 配置自动任务 -> 1 添加自动任务，可以看到末尾就有第《二》步添加的定时任务，输入对应的数字并回车后可设置执行条件
