...
- 2. 此方案彻底防止了 DNS 泄露（针对不在规则集内的域名和 IP 全部走 `fakeip` 或国外 DNS 解析），配置简单粗暴，兼容性无法保证，请慎用
...
```
{
  "dns": {
    ...
    "rules": [
      ...
      { "geosite": [ "private" ], "query_type": [ "A", "AAAA" ], "server": "dns_direct" },
