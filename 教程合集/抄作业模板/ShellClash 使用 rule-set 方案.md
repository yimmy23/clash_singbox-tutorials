...
2. 本模板适用于 [ShellClash](https://github.com/juewuy/ShellClash) 使用 rule-set 方案的模式
...
连接 SSH 后执行如下命令：
```
curl -o $clashdir/yamls/user.yaml -L https://cdn.jsdelivr.net/gh/DustinWin/clash-ruleset@main/shellclash-userconfig/redir-host-user.yaml
```
# 三、 导入配置文件
```
proxy-providers:
...
0 4 * * * curl -o /data/clash/yamls/user.yaml -L https://cdn.jsdelivr.net/gh/DustinWin/clash-geosite@release/fake-ip-user.yaml && /data/clash/start.sh restart >/dev/null 2>&1 #每天早上 4 点更新 user.yaml
```
