...
- 2. AdHuardHome **自带的添加黑名单列表**中的下载域名都是“adguardteam.github.io”，所以我定制的 user.yaml 中 `fake-ip-filter` 列表内含有 `- adguardteam.github.io` 域名，并添加了 `- https://anti-ad.net/easylist.txt`（可手动修改为此下载地址 URL，下载更稳定），黑名单可正常下载
